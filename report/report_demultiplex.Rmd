...
sum <- do.call(rbind, lapply(summaries, function(s) {
  ...
  sum <- data.frame(sum,
                    "Barcode" = splitted_path[length(splitted_path)],
...
```{r demux_summary}
demux_sum <- merge(demux, sum[c("read_id", "run_id", "Barcode", "Run_ID", "Demultiplexer")], by = c("read_id", "run_id"))
```
...
if (postdemux_folder != demux_folder) {
  ...
  postdemux_sum <- merge(postdemux, sum[c("read_id", "run_id", "Barcode", "Run_ID", "Demultiplexer")], by = c("read_id", "run_id"))
}
```
...
byrun <- all_sum %>%
  ...
  dplyr::group_by(Step, Demultiplexer, Run_ID, Barcode) %>%
...
ggplot(byrun, aes(x=Barcode, y=total.gigabases, fill=Demultiplexer, alpha = Step)) +
...
ggplot(byrun, aes(x=Barcode, y=median.length, fill=Demultiplexer, alpha = Step)) +
...
ggplot(byrun, aes(x=Barcode, y=N50.length, fill=Demultiplexer, alpha = Step)) +
  geom_col(position = position_dodge2(preserve = "single", padding = 0)) +
  scale_fill_viridis_d(option = "E", end = .85) +
  scale_alpha_discrete(range = c(.3, 1)) +
  facet_wrap(vars(Run_ID), scales = "fixed", ncol=3, strip.position = "right") +
...
ggplot(byrun, aes(x=Barcode, y=total.reads, fill=Demultiplexer, alpha = Step)) +
  geom_col(position = position_dodge2(preserve = "single", padding = 0)) +
  scale_fill_viridis_d(option = "E", end = .85) +
  scale_alpha_discrete(range = c(.3, 1)) +
  facet_wrap(vars(Run_ID), scales = "fixed", ncol=3, strip.position = "right") +
...
if (file.exists(bcgenome_tsv)) {
  ...
  bygenome <- merge(byrun, bcgenome, by = c("Demultiplexer", "Run_ID", "Barcode"))
}
```
...
if (file.exists(bcgenome_tsv)) {
  p <- ggplot(bygenome, aes(x=Genome_ID, y=total.gigabases, fill=Run_ID)) +
    ...
    geom_text(aes(label = Barcode), color = "#b1bed5", position = position_stack(), vjust = 1.2, size = 3.8*10/nlevels(as.factor(bcgenome$Genome_ID))) +
    scale_fill_viridis_d(option = "C") +
    theme_light() +
    theme(axis.text.x=element_text(angle=90,hjust=1),
          legend.direction = "vertical", legend.position = "top")

  if (!is.na(bygenome$Step)) p <- p + facet_wrap(vars(Step), nrow = 2)
  p
}
```
...
if (file.exists(bcgenome_tsv)) {
  ggplot(bygenome, aes(x=Genome_ID, y=median.length, fill=Run_ID)) +
    geom_col(position = position_dodge2(preserve = "single", padding = 0)) +
    geom_text(aes(label = Barcode), color = "#b1bed5", position = position_dodge2(width = 1, preserve = "single", padding = 0), angle = 90, vjust= .5, hjust = 1.1, size = 3.8*10/nlevels(as.factor(bcgenome$Genome_ID))) +
    scale_fill_viridis_d(option = "C") +
    theme_light() +
    theme(axis.text.x=element_text(angle=90,hjust=1),
          legend.direction = "vertical", legend.position = "top")
...
if (file.exists(bcgenome_tsv)) {
  ggplot(bygenome, aes(x=Genome_ID, y=N50.length, fill=Run_ID)) +
    geom_col(position = position_dodge2(preserve = "single", padding = 0)) +
    geom_text(aes(label = Barcode), color = "#b1bed5", position = position_dodge2(width = 1, preserve = "single", padding = 0), angle = 90, vjust = .5, hjust = 1.1, size = 3.8*10/nlevels(as.factor(bcgenome$Genome_ID))) +
    scale_fill_viridis_d(option = "C") +
    theme_light() +
...
if (file.exists(bcgenome_tsv)) {
  ggplot(bygenome, aes(x=Genome_ID, y=total.reads, fill=Run_ID)) +
    geom_col(position = position_stack(), color = "gray") +
    geom_text(aes(label = Barcode), color = "#b1bed5", position = position_stack(), vjust = 1.2, size = 3.8*10/nlevels(as.factor(bcgenome$Genome_ID))) +
    scale_fill_viridis_d(option = "C") +
    theme_light() +
