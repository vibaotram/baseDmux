...
.yaml` (generated by `baseDmux configure` - see below). It enables to setup input reads, output folder, parameters for the tools
 ...
 This actually corresponds to the typical Snakemake 'config.yaml' file. You can [take a look](baseDmux/data/config.yaml) at what serves as a template to create `profile/workflow_parameters.yaml`. It is suggested to **refer to the comments in this file for further details on individual parameters**.
