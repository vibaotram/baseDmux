...
rule report_demultiplex:
	input:
		...
		fastq = expand(os.path.join(outdir, "reads_per_genome/fastq{filtered}/{genome}{filtered}.fastq.gz"), genome = genome, filtered = filtered) if genome else expand(os.path.join(outdir, "demultiplex/{demultiplexer}/{run}/demultiplex.done"), demultiplexer = demultiplexer, run = run),
	message: "Reporting demultiplex results"
	output: os.path.join(outdir, "report/demultiplex_report.html")
	params:
		barcode_by_genome = BARCODE_BY_GENOME,
		fastq = os.path.join(outdir, "reads_per_genome/fastq") if genome else os.path.join(outdir, "basecall"),
		demultiplex = os.path.join(outdir, "demultiplex"),
		postdemux = expand(os.path.join(outdir, "reads_per_genome/fastq{filtered}"), filtered = filtered) if genome else os.path.join(outdir, "basecall"),
